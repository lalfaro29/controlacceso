...
function _update_method($position) {
	...
	switch ($position) {
		...
		case 'val':$updateMethod='val';break;
	}
...
function jq_on_press_enter_valor_entero($nombre,$opciones = array()){
	if(isset($opciones['with'])){
		$opciones['with'] .= "+'&'+$('#".$nombre."').attr('id')+'='+$('#".$nombre."').val()";
	}else{
		$opciones['with']  = "$('#".$nombre."').attr('id')+'='+$('#".$nombre."').val()";
	}
	$codigo = "$('#".$nombre."').keypress(function(e){
			if(e.keyCode == 13){
				if (!isNaN(this.value)){
					".jq_remote_function($opciones).";
				}else{
					alert('el valor ingresado no es numerico');
				}
			}
		});";
return jq_javascript_tag($codigo);
}

function jq_on_click($nombre,$opciones = array()){
	if(isset($opciones['with'])){
		$opciones['with'] .= "+'&'+$('#".$nombre."').attr('id')+'='+$('#".$nombre."').val()";
	}else{
		$opciones['with']  = "$('#".$nombre."').attr('id')+'='+$('#".$nombre."').val()";
	}
return jq_javascript_tag("$('#".$nombre."').click(function(e){".jq_remote_function($opciones).";});");
}
